...
class ClassiqueList extends StatefulWidget {
  const ClassiqueList({Key? key}) : super(key: key);

  @override
  _ClassiqueListState createState() => _ClassiqueListState();
}

class _ClassiqueListState extends State<ClassiqueList> {
  ...
  Widget build(BuildContext context) {
    ...
    final tabView = <Widget>[
      Container(child: buildListClassiqueAperitif()),
      Container(child: buildListClassiqueEntree()),
      Container(child: buildListClassiquePlat()),
      Container(child: buildListClassiqueDessert()),
    ];
  ...
  Widget buildListClassiqueAperitif() {
    var size = MediaQuery.of(context).size;
    return StreamBuilder<List<Recette>>(
      stream: getClassiqueRecetteAperitif(),
  ...
  Widget buildListClassiqueEntree() {
    var size = MediaQuery.of(context).size;
    return StreamBuilder<List<Recette>>(
      stream: getClassiqueRecetteEntree(),
      builder:(context, snapshot) {
        if (snapshot.hasError){return const Text('error');}
        else if (snapshot.hasData) {
          return ListView.builder(
  ...
  Widget buildListClassiquePlat() {
    var size = MediaQuery.of(context).size;
    return StreamBuilder<List<Recette>>(
      stream: getClassiqueRecettePlat(),
      builder:(context, snapshot) {
        if (snapshot.hasError){return const Text('error');}
        else if (snapshot.hasData) {
          return ListView.builder(
  ...
  Widget buildListClassiqueDessert() {
    var size = MediaQuery.of(context).size;
    return StreamBuilder<List<Recette>>(
      stream: getClassiqueRecetteDessert(),
      builder:(context, snapshot) {
        if (snapshot.hasError){return const Text('error');}
        else if (snapshot.hasData) {
          return ListView.builder(
